...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    if (!post) return res.status(400).json({ msg: 'There is no post found.' });
    res.json(post);
  } catch (err) {
    console.log(err.message);
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
  try {
    ...
    //Make sure somment exists
    if (!comment) {
      return res.status(404).json({ msg: 'No comment exist.' });
    }
